...
class db:
    ...
    def post_embeddings(self, values: list[tuple | dict]) -> bool:
        ...
        if isinstance(values[0], dict):
            sql = """
                ...
                VALUES (%(text)s, %(embedding)s,%(file_id)s)
            """
    ...
    def get_entities(
        self, entity_id: int | None = None, entity_name: str | None = None
    ):
        with self.conn:
            with self.conn.cursor() as curr:
    ...
    def add_file(
        self, filename: str, entity: int, category: str, embedding: list[float]
    ):
    ...
    def get_files(self, file_id: int = 0) -> pd.DataFrame:
        ...
        with self.conn:
            with self.conn.cursor() as curr:
                sql = """
                    SELECT id, name, entity_id, category, uploaded_at, embedding 
                    FROM files;
                """
                param = None
                if file_id:
        ...
        with self.conn:
            with self.conn.cursor() as curr:
                sub_query = """
                    ...
                    FROM files 
                    JOIN entities
                    ON entities.id = files.entity_id 
                    WHERE entities.id = %s
                """
